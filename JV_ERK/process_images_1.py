...
input_dir = '/scratch/gpfs/ksuh/ERK_FUCCI/1/'
output_dir = '/scratch/gpfs/ksuh/ERK_FUCCI/Result/'

# Path to working directory (where weights/stats are saved):
working_dir = '/home/ksuh/JulieVision8bitCrop/Leica_20x/'

# Weights file name:
weights_file = 'Leica_20X_weights_True.h5'
...
def process_one_image(impath, i, img, h, w, pred_img, model, stepSize, windowSize, output_mean, output_stdev, img_shape, bord):
    # Initialize arrays: 
    ...
    pred_img_temp = output_dir + 'out_' + impath 
...
for i in range(len(impathlist)):
    ...
    [h, w] = np.shape(img)
    process_one_image(impath, i, img, h, w, pred_img, model, stepSize, windowSize, out_mean, out_stdev, img_shape, bord)
